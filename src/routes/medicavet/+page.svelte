<script>
	...
	import { MedicavetSearchResults, MedicavetMinisearch, MedicavetSearchValue } from '$lib/stores';
	import { slide } from 'svelte/transition';
	import angleDown from 'svelte-awesome/icons/angleDown';
	import angleUp from 'svelte-awesome/icons/angleUp';
	import medAutocomplete from '$lib/medicavet_autocomplete.json';
	...
	const runSearch = () => {
		...
		if ($MedicavetSearchValue.length > 0) {
			$MedicavetSearchResults = $MedicavetMinisearch.search($MedicavetSearchValue).slice(0, 200);
			if ($MedicavetSearchResults.length === 0) {
	...
	onMount(async () => {
		if ($MedicavetMinisearch == undefined) {
			try {
				const response = await import('$lib/medicavet.json');

				medicaments = response.default;
			} catch (error) {
				console.error('Failed to load JSON:', error);
			} finally {
				$MedicavetMinisearch = new MiniSearch({
					fields: [
						'Spécialité',
						'Dosage',
						'Forme',
				...
				$MedicavetMinisearch.addAll(medicaments);
				isLoading = false;
			}
		} else {
			isLoading = false;
	...
	const handleChangeAutocomplete = () => {
		$MedicavetSearchValue = autocompleteItems[autocompleteIndex];
		showAutocomplete = false;
		runSearch();
	};
	...
	function filterStrings(array, searchString) {
		...
		return array
			...
			.filter((item) => item.normalized.includes(normalizedSearch))
			.sort((a, b) => {
	...
	$: expandedstates = $MedicavetSearchResults.map(() => false);
	$: autocompleteItems = filterStrings(medAutocomplete, $MedicavetSearchValue);
</script>

<div class="w-full space-y-8 px-4 py-8 grid">
	{#if isLoading}
		<p class="p-4 text-center">Chargement de Médicavet</p>
		<ProgressBar value={undefined} meter="bg-secondary-500" track="bg-secondary-500/30" />
	{:else}
		<h1 class="h1 text-center">Médicavet</h1>
		<div class="text-sm text-center">
			<div>Un petit moteur de recherche de médicaments vétérinaires disponibles en Tunisie.</div>
		...
		<div>
			<div class="input-group input-group-divider grid-cols-[1fr_auto] xl:w-2/3 mx-auto">
				<input
					type="search"
					bind:value={$MedicavetSearchValue}
					on:keydown={(event) => {
						if (event.key === 'Enter') {
							runSearch();
						}
			...
			<div class="xl:w-2/3 mx-auto">
				{#if $MedicavetSearchValue.length > 0 && showAutocomplete && autocompleteItems.length > 1}
					<ListBox
		...
		<div class="space-y-4 grid">
			{#if $MedicavetSearchResults.length > 0}
				{#each $MedicavetSearchResults as result, index}
...
<svelte:head>
	<title>Médicavet</title>
	<meta name="description" content="Medicavet est un petit moteur de recherche de médicaments." />
</svelte:head>
